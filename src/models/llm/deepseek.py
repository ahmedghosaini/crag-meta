...
You are a helpful, respectful, and honest question-answering system. I will be providing you with questions, as well as some additional context that may be helpful in answering those questions. You will be provided with additional information between the "<DOCS>" tags.

Keep your answers brief, ideally less than 20 words, but with a strict limit of 30 words.

    If the provided information is insufficient for answering the question, simply output "Insufficient information" and nothing else.

    If the question asserts a false premise, like "When did Eisenhower become Prime Minister?", simply output "Invalid question".

        If you are inclined to say something like "X never has", "X never was", or "X never did", your output should instead be "Invalid question".

    A final reminder regarding award shows: Years can be tricky. Often, awards are handed out the year after the project was made. For example, the Oscars in 2015 recognize movies made in 2014.
""".strip()
...
You are a helpful, respectful, and honest question-answering system. Your job is to answer questions that you can confidently answer without requiring additional context. This includes topics you have knowledge of, such as simple facts, reasoning-based questions, and common sense inquiries.

Keep your answers brief—responses must be under 50 words.

    If a question asserts a false premise (e.g., "When did Eisenhower become Prime Minister?"), respond with "Invalid question".

    If you do not know the answer, respond with "I don't know".

    For date-related questions (e.g., "What date did X happen?"), always respond with "I don't know" to avoid inaccuracies.
""".strip()


API_SYSTEM_INSTRUCTIONS = """
You are a system that predicts the correct Python function to call with the appropriate arguments. You will receive a query along with documentation for a set of Python functions that interact with a remote API and return results.

Your task is to:

    Analyze the query.

    Select the most relevant function and provide the correct arguments.

    If no function is suitable, return "None".

Output Format:
Your response will be directly evaluated using eval(), so return only the function call with its arguments, without any additional text or explanations.
""".strip()
...
You are a precise and reliable question-answering system. You will receive an "ORIGINAL QUESTION" along with a "QUESTION" that asks whether "<answer 1>" is equivalent to "<answer 2>" based on suitability.

    If the answers are equivalent, output: "ANSWER: 1"

    If they are not equivalent, output: "ANSWER: 0"

Provide only the final answer without any additional text.
""".strip()
...
class DeepSeek:
    def __init__(self, model_name=MODELS_DIR / "DeepSeek-R1-Distill-Llama-8B", peft_path=MODELS_DIR / "peft_task3", api_peft_path=MODELS_DIR / "peft_api", use_peft=True, qa_prompt=QA_SYSTEM_INSTRUCTIONS, api_prompt=API_SYSTEM_INSTRUCTIONS ,bnb_config=DEFAULT_CONFIG, batch_size=1, prompt_format="v1", include_context_examples=True):
    ...
    def process_candiates(self, query, query_time, candidates):
        ...
        with torch.no_grad():
            ...
            result = self.generate_response(formatted_text)
            # result = result[0]["generated_text"].split("<|start_header_id|>assistant<|end_header_id|>")[-1].strip()
            # result = result[0]["generated_text"].split("ANSWER:")[-1].strip()
            result = result.split("ANSWERABLE:")[-1].split("</think>")[0].strip()
