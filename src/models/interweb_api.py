...
def prompt_format_v0(prompt, query, query_time, candidates, include_context_examples, token_budget=3500):
    bos = "<|begin_of_text|>"
...
def prompt_format_v1(prompt, query, query_time, candidates, include_context_examples, token_budget=3500):
    bos = "<|begin_of_text|>"
    sys_message = "<|start_header_id|>system<|end_header_id|>"
    eos = "<|eot_id|>"
    user_message = "<|start_header_id|>user<|end_header_id|>"
...
class ApiCall:
    ...
    def process_candiates(self, query, query_time, candidates):
        if self.prompt_format == "v0":
            formatted_text = prompt_format_v0(self.qa_prompt, query, query_time, candidates, self.include_context_examples)
        elif self.prompt_format == "v1":
            formatted_text = prompt_format_v1(self.qa_prompt, query, query_time, candidates, self.include_context_examples)
        else:
            formatted_text = prompt_format_no_context(query, query_time, self.include_context_examples)
        ...
        completion = self.client.chat.completions.create(
			model=self.model_name, # gpt-35-turbo
