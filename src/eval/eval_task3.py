...
def parse_args():
    ...
    parser.add_argument("--num_top_docs", type=int, default=20)
    parser.add_argument("--use_test_set", action="store_true")
    parser.add_argument("--use_peft", type=str2bool, default=True)
    parser.add_argument("--peft_path", default="models/peft_v3")
...
def main():
    ...
    if args.use_test_set:
        ...
        json_objs = []
        for file in sorted(Path("data/raw/task3").glob("*.jsonl")):
            print(file)
            json_objs.extend(read_jsonl(str(file), -1))
    ...
    results = []
    for k, json_obj in tqdm(enumerate(json_objs)):
        # if not Path(f"partial_jsons/{k}.json").is_file():
        if not Path(f"partial_jsons/none.json").is_file():
            ...
            pred_answer, candidates_text_representation = pipeline.generate_answer_task3(query, search_results, query_time, True)
            results.append(
                {
                    "query": query,
                    ...
                    "partial_prompt": candidates_text_representation
                }
            )
            with open(f"partial_jsons/{k}.json", "w") as f:
